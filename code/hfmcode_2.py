...
def getjhd(filename):
    ...
    ajpl = dict(zip(keyliste,value))
    jhd = {a:round(b*jhzs[a],4) for a, b in ajpl.items()}
    guiyi_jhd = list(jhd.values())
    sum_jhd = round(sum(guiyi_jhd),4)
    return sum_jhd
...
if __name__ == '__main__':
    ...
    str_hfmcode += '\n附件1: 哈夫曼编码表（格式为 \'全拼编码\':[出现次数,\'哈夫曼编码\']）\n'+str(addedlist)
    str_hfmcode += '\n\n附件2: 编码结果\n' +a
    str_hfmcode += '\n\n附件3: 解码结果\n' +aa
